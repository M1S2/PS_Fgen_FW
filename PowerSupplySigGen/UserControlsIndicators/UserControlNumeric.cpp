...
template <class T>
UserControlNumeric<T>::UserControlNumeric(uint8_t locx, uint8_t locy, T* controlValue, const char* baseUnit, int8_t valueStoreUnitPower, T minValue, T maxValue, void* valueChangedContext, void (*onValueChanged)(void*), const u8g_pgm_uint8_t* iconBits) : UserControlBase(locx, locy, valueChangedContext, onValueChanged, iconBits)
{
        _baseUnit = baseUnit;
...
void UserControlNumeric<T>::Draw(u8g_t *u8g, bool isFirstPage)
{
	...
	u8g_DrawStr(u8g, this->_locX + CONTROLS_ICON_SIZE + 4 - (_displayValue < 0 ? 1 : 0), this->_locY + 3 + CONTROLS_FONT_HEIGHT, stringBuffer);
			
	if(this->IsActive)
	{
		u8g_uint_t cursorXpos = this->_locX + CONTROLS_ICON_SIZE + 1 + (-_currentDigitPosition + 3) * 6 + (_currentDigitPosition < 0 ? 3 : 0);
