...
template <class T>
UserControlEnum<T>::UserControlEnum(uint8_t locx, uint8_t locy, T* controlValue, const char** enumNames, uint8_t numEnumValues, void* valueChangedContext, void (*onValueChanged)(void*), const u8g_pgm_uint8_t* iconBits) : UserControlBase(locx, locy, valueChangedContext, onValueChanged, iconBits)
{
...
void UserControlEnum<T>::Draw(u8g_t *u8g, bool isFirstPage)
{
	...
	u8g_DrawStr(u8g, this->_locX + CONTROLS_ICON_SIZE + 4, this->_locY + 3 + CONTROLS_FONT_HEIGHT, _enumNames[_controlValueDraw]);
}