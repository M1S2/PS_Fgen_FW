...
const char* PsRegulationModesNames[] = { "FIX", "CV" };


#ifdef PS_SUBSYSTEM_ENABLED

const char* PsStatesNames[] = { "CV", "OVL", "OVP", "OCP", "OPP" };

PS_Channel::PS_Channel(float minVolt, float maxVolt, uint8_t minOvpLevel, uint8_t maxOvpLevel, float minOvpDelay, float maxOvpDelay, float minOcpLevel, float maxOcpLevel, float minOcpDelay, float maxOcpDelay, float minOppLevel, float maxOppLevel, float minOppDelay, float maxOppDelay) : Channel(POWER_SUPPLY_CHANNEL_TYPE)
{
	...
	OcpLevel = Parameter<float>(0, minOcpLevel, maxOcpLevel, 0.5, 0.1);
...
{
	if(GetEnabled() && (PsState == PS_STATE_CV || PsState == PS_STATE_OVL))
	{
...
void PS_Channel::CheckProtections()
{
	...
	if(GetOcpState() && MeasuredCurrent > GetOcpLevel())
	{
